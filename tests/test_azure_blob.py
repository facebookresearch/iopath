...
class TestContext(NamedTuple):
    ...
    def get_local_test_file(self, name: str, length: int = 4096) -> str:
...
class TestAzureBlob(unittest.TestCase):
    ...
    def get_container_path(self, base_path: str) -> str:
    ...
    def get_md5(self, local_path: str) -> str:
        with open(local_path, "rb") as f:
            return self.get_md5_from_file(f)

    # pyre-ignore[2]: f is a file-like object
    def get_md5_from_file(self, f) -> str:
        return hashlib.md5(f.read()).hexdigest()

    @contextmanager
    def context(self) -> Generator[TestContext, None, None]:
        """
    ...
    def test_parse_uri_when_valid(self) -> None:
        valid_uris = {
    ...
    def test_parse_uri_invalid_protocol(self) -> None:
        with self.context() as ctx:
            try:
                uri = "s3://bucket/obj"
    ...
    def test_status_operations(self) -> None:
        with self.context() as ctx:
            handler = ctx.handler
